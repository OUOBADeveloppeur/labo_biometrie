...
import com.hf.passport.other.FrameMetadata;
...
public class TextRecognitionProcessor {
    ...
    public static final String ID_CARD_TD_1_LINE_1_REGEX = "([A|C|I][A-Z0-9<]{1})([A-Z]{3})([A-Z0-9<]{31,33})";
    ...
    public static final String PASSPORT_TD_3_LINE_1_REGEX = "(P[A-Z0-9<]{1})([A-Z]{3})([A-Z0-9<]{39,41})";
    public static final String PASSPORT_TD_3_LINE_1_REGEX_SHORT = "(P[A-Z0-9<]{1})([A-Z]{3})([A-Z0-9<]{12,41})";
    ...
    protected final ArrayBlockingQueue<InputImage> mInputImageQueue;
    private RecognizeThread mRecognizeThread;
    protected boolean isRecognizeInterrupt;

    public TextRecognitionProcessor(DocType docType, ResultListener resultListener) {
        this.docType = docType;
        this.resultListener = resultListener;
        IDAnalyser.getInstance().setResultCallback(resultListener);
        mInputImageQueue = new ArrayBlockingQueue<>(5);
        mRecognizeThread = new RecognizeThread();
        mRecognizeThread.start();
    }

    public void release() {
        if (null != mRecognizeThread) {
            isRecognizeInterrupt = true;
            mRecognizeThread.interrupt();
        }
        IDAnalyser.getInstance().close();
    }
    ...
    public void start() {
        startProcess = true;
        IDAnalyser.getInstance().start();
    }

    public void stop() {
        startProcess = false;
        IDAnalyser.getInstance().stop();
    }
//    public void stop() {
...
//    }
    ...
    public void process(ByteBuffer data, FrameMetadata frameMetadata, GraphicOverlay graphicOverlay) throws MlKitException {
        ...
        mInputImageQueue.offer(inputImage);
//        IDAnalyser.getInstance().onBitmapPrepared2(inputImage);
...
//        return textRecognizer.process(image);
    ...
    private void finishScanning(final MRZInfo mrzInfo) {
        try {
            if (isMrzValid(mrzInfo)) {
                ...
                new Handler().postDelayed(() -> resultListener.onSuccess(mrzInfo, null), 1000);
            }
    ...
    protected class RecognizeThread extends Thread {
    ...
    protected void recognizeProcess() {
        while (!isRecognizeInterrupt) {
            InputImage inputImage;
            try {
                inputImage = mInputImageQueue.take();
                IDAnalyser.getInstance().onBitmapPrepared3(inputImage);
