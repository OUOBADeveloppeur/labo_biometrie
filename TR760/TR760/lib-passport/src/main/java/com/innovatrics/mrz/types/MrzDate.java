...
public class MrzDate implements Serializable, Comparable<MrzDate> {
    ...
    private boolean check() {
        if (year < 0 || year > 99) {
            Log.d("","Parameter year: invalid value " + year + ": must be 0..99");
            return false;
        }
        if (month < 1 || month > 12) {
            Log.d("","Parameter month: invalid value " + month + ": must be 1..12");
            return false;
        }
        if (day < 1 || day > 31) {
            Log.d("","Parameter day: invalid value " + day + ": must be 1..31");
            return false;
        }
